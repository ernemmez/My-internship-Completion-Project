...
Kullanıcı, site ve üçüncü taraf sitelerdeki şifre ve hesap güvenliğinden kendisi sorumludur. Aksi halde oluşacak veri kayıplarından ve güvenlik ihlallerinden veya donanım ve cihazların zarar görmesinden Firma sorumlu tutulamaz.</p>
							...
							<div class="col-md-4" style="margin: auto;margin-left: -310px;margin-top: 150px;">
								<form action="" method="post">
								  ...
								  <button type="submit" class="btn btn-primary" name="girisyap" style="background-color: #ee6363;">Giriş Yap</button>
								</form>
							</div>
							<?php 
					  if ($_POST) {
					    ...
					    if ($sonuc) {
					      if ($sifre==$sonuc["sifre"]) {
					        session_start();
					         ...
					         echo "<script>alert('Merhaba Sayın $kullanici,Hoşgeldiniz.')</script>";
					         echo "<script>window.location.href='index.php'</script>";
					    ...
					    {
					    echo "<script>alert('Bu Kullanıcı Mevcut Değil!')</script>";
					    }
					  }
